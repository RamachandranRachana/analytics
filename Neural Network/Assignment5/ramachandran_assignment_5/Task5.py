...
class NLayerNeuralNetwork:
    ...
    def buildtrainingmodel(self):
        ...
        plt.savefig("confusion_mat_task5.png")
        plt.clf()
        yl = err_list
        xl = epoch_count
        plt.title("error/epoches")
        plt.scatter(xl, yl)
        plt.savefig("error_rate_task5.png")
        ...
        plt.scatter(xl, yl)
        plt.savefig("loss_rate_task5.png")
...
if __name__ == "__main__":
    train_labels=create_samples(path_train_1000)
    train=train_labels[0]
    trlabels=train_labels[1]
    test_labels=create_samples(path_test_1000)
    test = test_labels[0]
