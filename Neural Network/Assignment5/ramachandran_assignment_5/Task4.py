...
class NLayerNeuralNetwork:
    def __init__(self,train,test,trlabels,tslabels):
        ...
        self.initialize()
    def initialize(self):
        self.buildtrainingmodel(lam_da=0.1)
        self.buildtrainingmodel(lam_da=0.2)
        self.buildtrainingmodel(lam_da=0.3)
        self.buildtrainingmodel(lam_da=0.4)
        self.buildtrainingmodel(lam_da=0.5)
    def buildtrainingmodel(self,lam_da):
        ...
        loss=(T.nnet.categorical_crossentropy(probabilities,tar).mean()+ loss_reg)/num
        ...
        epoch_count = []
        for i in range(100):
             err, output = train(btrain,self.target_train)
             print i
             if i % 20 == 0:
                 print "loss: ", err
                 epoch_count.append(i)
                 loss_list.append(round(err,5))
