...
import colorsys
import read_one_image_and_convert_to_vector as imrd
import numpy as np
import scipy.misc
import os
...
def create_target_vector(file_name):
...
class ClNNExperiment:
    ...
    Rachana Ramachandran 2016_09_18
    """
    ...
    def read_samples(self):
        ...
        for filename in os.listdir(path):
            ...
            image_vector = imrd.read_one_image_and_convert_to_vector(os.path.join(path, filename))
            normalize_vector=np.true_divide(image_vector,255)
            #print normalize_vector
            sample.append(normalize_vector)
            tv = create_target_vector(filename)
            target.append(tv)
...
class ClNNGui2d:
    ...
    def __init__(self, master, nn_experiment):
        ...
        self.hebbianrule_variable=Tk.StringVar()
        self.hebbianrule_button = Tk.OptionMenu(self.buttons_frame, self.hebbianrule_variable, "FilteredLearning",
                                                      "DeltaRule", "UnsupervisedHebb",
                                                      command=lambda event: self.hebbianrule_button_callback())
        self.hebbianrule_variable.set("learningRules")
        self.hebbianrule_button.grid(row=3,column=0,sticky=Tk.N + Tk.E + Tk.S + Tk.W)
        self.initialize()
        self.refresh_display()

    def initialize(self):
        self.nn_experiment.create_samples()
        #self.nn_experiment.read_samples()
    ...
    def print_nn_parameters_button_callback(self):
        ...
        self.print_nn_parameters_button.update_idletasks()
    def hebbianrule_button_callback(self):
        self.nn_experiment.hebbianlearningrule=self.hebbianrule_variable.get()
        self.nn_experiment.read_samples()
        #self.nn_experiment.target_sample()
        self.nn_experiment.adjust_weights()
        self.refresh_display()
...
class ClNeuralNetwork:
    ...
    def adjust_weights(self, input_samples, error):
        ...
        #print ecol
        for layer in self.layers:
            wmat=layer.weights.shape
            wrow=wmat[0]
            wcol=wmat[1]
            #print wrow
            #print wcol
            print layer.weights
        xrange=input_samples[0]
        yrange=input_samples[1]
        p=[]
        for x in xrange:
            for y in yrange:
                p.append([x,y,1])
        ...
        for j in range(erow):
            for i in range(ecol):
                for layer in self.layers:
                    #layer.weights[j]+=[x*error[j][i] for x in p[i]]
                     if error[j][i]==0.:
        ...
        if self.experiment.hebbianlearningrule=="FilteredLearning":
            print "case1"
        elif self.experiment.hebbianlearningrule=="DeltaRule":
            print "case2"
        elif self.experiment.hebbianlearningrule=="UnsupervisedHebb":
            print "case3"
